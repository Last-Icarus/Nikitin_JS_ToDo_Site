...
function TodoList(props) {
    const [currentPage, setCurrentPage] = useState(1); //Хук состояния, выставляющий параметры по умолчанию
    ...
    if (list !== undefined) {
        ...
        <>
            <Col span={8} style={{margin: '0 auto'}}>
                <h3>Список дел {props.uid ? '#' + props.uid : ''}</h3>
                <List>
                    <Input placeholder='Добавить дело в список' onPressEnter={(e) => handlerAddToTodo(e)}/>
                ...
                <Pagination locale={{ items_per_page: "элементов на страницу", jump_to: "Перейти на" , page: 'страницу'}} size="defaul" showSizeChanger showQuickJumper defaultCurrent={currentPage} defaultPageSize={perPage} total={showList.length} onChange={handlerUpdatePages} style={{paddingBottom: 16}} />
            </Col>
        </>
        );
    }
